...
function push() {
    ...
    while (now <= pkgnum) {
        ...
        if (multi) {
            xhr.onreadystatechange = change(now, timestamp1, xhr);
        }
        else {
...
function both(notboth = "服务器上无重复", both = "服务器上有重复", upload = false, back = false) {
    var xhrb = new XMLHttpRequest();
    xhrb.open("POST", php, false);
    xhrb.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
    xhrb.send("job=both&docname=" + docname);
    if (back) {
        var t = 0;
        while (t < 1000 && xhrb.responseText == "notboth") {
            xhrb.open("POST", php, false);
            xhrb.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
            xhrb.send("job=both&docname=" + docname);
            t += 1;
        }
        if (xhrb.responseText == "notboth") {
            alert("检查上传超时!可能未上传成功！");
            message.innerHTML = message.innerHTML + "<br />检查上传超时!可能未上传成功!";
            message.innerHTML = message.innerHTML + "<br />请检查开发者模式是否仍在上传";
        } else {
            alert("上传成功!");
            message.innerHTML = message.innerHTML + "<br />3秒后自动返回";
            window.setTimeout("window.location='/doc/'", 3000);
        }
    } else {
        if (xhrb.responseText == "notboth") {
